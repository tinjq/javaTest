...
public class TestNonBlockingNIO3 {
	...
	public static void send(String content) throws IOException{
		...
		buf.put((new SimpleDateFormat("yyyy-MM-dd HH:mm:ss,SSS").format(new Date()) + "\t" + content).getBytes());
		buf.flip();
		sChannel.write(buf);
		buf.clear();
			...
			while(it.hasNext()){
				...
				if(sk.isAcceptable()){
					...
					while((len = sChannel.read(buf)) > 0 ){
						buf.flip();
						String cContent = new String(buf.array(), 0, len);
						System.out.println(cContent);
						buf.clear();
						
						// System.out.println("aaa");
						buf.put(("Hello " +  cContent).getBytes());
						sChannel.write(buf);
						buf.clear();
					}
